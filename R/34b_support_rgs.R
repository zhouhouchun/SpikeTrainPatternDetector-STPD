# RGS_core.R
# -----------------------------------------------------------------------------
# Robust Gaussian Surprise (RGS) burst and pause detector with a Shiny front end.
#
# Scientific scope
...
RGS_IMPLEMENTATION_VERSION <- "1.0.1"
...
rgs_validate_parameters <- function(params = rgs_default_parameters()) {
  ...
  params$alpha <- rgs_as_scalar_numeric(params$alpha, "alpha", 0, 1,
                                        lower_open = TRUE, upper_open = TRUE)
...
rgs_reference_diagnostics <- function(values, group, params, train_n) {
  ...
  if (length(central) < 10L) {
    rgs_stop("Reference group '", group,
             "' has fewer than 10 central NLISIs after trimming.")
  }
  ...
  data.frame(
    ...
    warning = if (is.finite(qq_cor) && qq_cor < 0.97) {
...
rgs_fit <- function(spike_trains, groups = NULL,
  ...
  for (group in unique_groups) {
    ...
    diag$central_pair_n <- nrow(central_pairs)
    diag$central_lag1_correlation <- lag1
    if (is.finite(lag1) && abs(lag1) > 0.20) {
      note <- "Central NLISI lag-1 dependence is non-negligible; Gaussian-sum independence is questionable."
...
rgs_finalize_candidates <- function(candidates, params, event_type) {
  ...
  k <- sum(overlap_selected & candidates$raw_p < params$alpha, na.rm = TRUE)
  ...
  accepted <- overlap_selected & adjusted < params$alpha
...
rgs_write_exports <- function(analysis, out_dir) {
  ...
  saveRDS(analysis, file.path(out_dir, "RGS_analysis.rds"))
  invisible(out_dir)
}
...
stpd_rgs_threshold_table <- function(analysis) {
  ...
  rows <- lapply(names(refs), function(group) {
    ...
    data.frame(
      reference_group = group,
      threshold_name = c("central_mu", "central_sigma", "burst_seed_threshold",
                         "pause_seed_threshold", "alpha"),
      threshold_value = c(ref$mu, ref$sigma, ref$burst_seed_threshold,
                          ref$pause_seed_threshold, analysis$parameters$alpha),
      threshold_unit = "normalized_log_ISI",
      threshold_source = c(rep("fitted_reference", 4L), "declared_parameter"),
      comparison_operator = c("reported", "reported", "lt", "gt", "adjusted_p_lt"),
...
stpd_rgs_support_export <- function(support, out_dir) {
  ...
  saveRDS(support$analysis, file.path(out_dir, "RGS_analysis.rds"))
