  ...
  div(
    ...
    mainPanel(
      ...
      tabsetPanel(
                       ...
                       column(3,
                              ...
                              selectInput("support_method_view", "\u652F\u6301\u65B9\u6CD5\u9762\u677F",
                                          choices = c("Mean-ISI \u6587\u7AE0\u65B9\u6CD5" = "misi",
                                                      ...
                                                      "Poisson Surprise (PS)" = "ps",
                                                      "Robust Gaussian Surprise (RGS)" = "rgs"),
                                          selected = "misi"),
                              ...
                              checkboxGroupInput("support_overlay_methods", "\u53E0\u52A0\u652F\u6301\u65B9\u6CD5\u68C0\u6D4B\u7ED3\u679C",
                                                 choices = c("Mean-ISI" = "misi", "LogISI / newBD" = "logisi",
                                                             "Poisson Surprise" = "ps", "RGS Burst" = "rgs"),
                              ...
                              conditionalPanel("input.support_method_view == 'ps'",
                                tags$hr(),
                                h4("Poisson Surprise \u652F\u6301"),
                                tags$div(class = "small-note",
                                         "pCLAMP-style Legendy-Salcman \u53D8\u4F53\uFF1B\u4EC5\u751F\u6210 Burst \u8F85\u52A9\u8BC1\u636E\uFF0C\u4E0D\u6539\u5199 STPD AUTO \u6807\u7B7E\u3002"),
                                checkboxInput("ps_support_visible_only", "\u4EC5\u5728\u5F53\u524D\u53EF\u89C1 trains \u4E0A\u8FD0\u884C", TRUE),
                                numericInput("ps_surprise_threshold", "Surprise \u9608\u503C", value = 10, min = 0, step = 0.5),
                                selectInput("ps_log_base", "Surprise \u5BF9\u6570\u5E95", choices = c("Natural log" = "e", "log10" = "10"), selected = "e"),
                                numericInput("ps_min_spikes", "Burst \u6700\u5C0F spike \u6570", value = 3, min = 3, step = 1),
                                numericInput("ps_seed_isi_factor", "Seed ISI / train mean ISI", value = 0.5, min = 0.01, step = 0.05),
                                numericInput("ps_rejection_isi_factor", "\u505C\u6B62 ISI / train mean ISI", value = 2, min = 0.1, step = 0.1),
                                numericInput("ps_max_failed_additions", "\u6700\u5927\u8FDE\u7EED\u65E0\u6539\u5584\u6269\u5C55", value = 10, min = 1, step = 1),
                                actionButton("run_ps_support", "\u8FD0\u884C Poisson Surprise \u652F\u6301", width = "100%"),
                                uiOutput("ps_support_status"),
                                downloadButton("download_ps_support_zip", "\u4E0B\u8F7D PS \u652F\u6301 ZIP", width = "100%")
                              ),
                              conditionalPanel("input.support_method_view == 'rgs'",
                                tags$hr(),
                                h4("Robust Gaussian Surprise \u652F\u6301"),
                                tags$div(class = "small-note",
                                         "RGS \u5728\u9884\u5148\u58F0\u660E\u7684 reference group \u5185\u62DF\u5408 normalized log-ISI\uFF0C\u540C\u65F6\u751F\u6210 Burst/Pause \u8F85\u52A9\u8BC1\u636E\u3002"),
                                checkboxInput("rgs_support_visible_only", "\u4EC5\u5728\u5F53\u524D\u53EF\u89C1 trains \u4E0A\u8FD0\u884C", TRUE),
                                selectInput("rgs_calibration_mode", "Reference \u6A21\u5F0F",
                                            choices = c("\u540C\u6570\u636E reference\uFF08exploratory\uFF09" = "same_data",
                                                        "\u9884\u5B9A calibration trains -> held-out prediction" = "held_out"),
                                            selected = "same_data"),
                                conditionalPanel("input.rgs_calibration_mode == 'held_out'",
                                  selectizeInput("rgs_calibration_trains", "Calibration trains", choices = NULL, multiple = TRUE)
                                ),
                                selectInput("rgs_group_source", "Reference group \u6765\u6E90",
                                            choices = c("\u5168\u90E8\u4E3A\u4E00\u7EC4" = "single",
                                                        "Train metadata \u5217" = "metadata",
                                                        "\u4E0A\u4F20 train-group CSV" = "upload",
                                                        "\u624B\u5DE5\u7F16\u8F91 train-group \u8868" = "manual"),
                                            selected = "single"),
                                conditionalPanel("input.rgs_group_source == 'metadata'", uiOutput("rgs_metadata_group_control")),
                                conditionalPanel("input.rgs_group_source == 'upload'",
                                  fileInput("rgs_group_map_upload", "Train-group CSV", accept = c(".csv", "text/csv"))
                                ),
                                conditionalPanel("input.rgs_group_source == 'manual'", DTOutput("rgs_group_editor")),
                                selectInput("rgs_scientific_mode", "Scientific mode",
                                            choices = c("Exploratory" = "exploratory", "Publication (fail closed)" = "publication"),
                                            selected = "exploratory"),
                                numericInput("rgs_candidate_count_p", "K \u5019\u9009 raw-P \u9608\u503C", value = 0.05, min = 0.0001, max = 0.5, step = 0.01),
                                numericInput("rgs_familywise_alpha", "Family-wise alpha", value = 0.05, min = 0.0001, max = 0.5, step = 0.01),
                                numericInput("rgs_window_fraction", "\u5C40\u90E8\u79FB\u52A8\u7A97\u6BD4\u4F8B", value = 0.20, min = 0.01, max = 0.49, step = 0.01),
                                selectInput("rgs_mad_scale", "MAD \u5B9A\u4E49", choices = c("normal-consistent MAD" = "normal_consistent", "raw MAD" = "raw_mad"), selected = "normal_consistent"),
                                numericInput("rgs_min_burst_spikes", "Burst \u6700\u5C0F spike \u6570", value = 3, min = 2, step = 1),
                                numericInput("rgs_min_pause_spikes", "Pause \u6700\u5C0F\u8FB9\u754C spike \u6570", value = 2, min = 2, step = 1),
                                actionButton("run_rgs_support", "\u8FD0\u884C RGS \u652F\u6301", width = "100%"),
                                uiOutput("rgs_support_status"),
                                downloadButton("download_rgs_support_zip", "\u4E0B\u8F7D RGS \u652F\u6301 ZIP", width = "100%")
                              )
                       ),
                       column(9,
                              ...
                              tabsetPanel(
                                ...
                                tabPanel("PS",
                                  h4("PS Burst \u4E8B\u4EF6"),
                                  DTOutput("ps_burst_table"),
                                  tags$hr(),
                                  h4("PS \u5019\u9009\u5BA1\u8BA1"),
                                  DTOutput("ps_candidate_table"),
                                  tags$hr(),
                                  h4("PS \u9608\u503C\u4E0E\u6765\u6E90"),
                                  DTOutput("ps_threshold_table")
                                ),
                                tabPanel("RGS",
                                  h4("RGS \u652F\u6301\u6458\u8981"),
                                  DTOutput("rgs_support_summary_table"),
                                  tags$hr(),
                                  h4("RGS Burst / Pause \u4E8B\u4EF6"),
                                  DTOutput("rgs_burst_table"),
                                  DTOutput("rgs_pause_table"),
                                  tags$hr(),
                                  h4("RGS reference diagnostics"),
                                  DTOutput("rgs_reference_diagnostics_table")
                                )
                              ))
                     ))),
        ...
        tabPanel("\u591A\u7B97\u6CD5\u7EA0\u5BDF\u4E0E\u4EBA\u5DE5\u6807\u6CE8",
                 tags$style(HTML("\n                   .patrol-card{background:white;border:1px solid #D8E1EC;border-radius:10px;padding:12px 14px;margin-bottom:12px}\n                   .patrol-title{font-weight:700;font-size:17px}\n                   .patrol-note{color:#53657D;font-size:13px;line-height:1.45}\n                   .patrol-pill{display:inline-block;padding:3px 8px;margin:2px;border-radius:999px;background:#EEF3F8;font-size:12px}\n                 ")),
                 tags$div(
                   class = "stpd-estimand-notice",
                   tags$strong("\u591A\u65B9\u6CD5\u8BC1\u636E\u53EA\u7528\u4E8E\u7EA0\u5BDF\u4E0E\u4EBA\u5DE5\u590D\u6838\u3002"),
                   " \u4EBA\u5DE5\u4FEE\u8BA2\u4FDD\u5B58\u5728\u72EC\u7ACB\u3001\u53EF\u8FFD\u6EAF\u7684\u8BB0\u5F55\u5C42\uFF0C\u4E0D\u6539\u5199 STPD AUTO \u8F93\u51FA\u6216\u539F\u59CB\u4EBA\u5DE5\u53C2\u8003\u3002"
                 ),
                 fluidRow(
                   column(
                     4,
                     actionButton("patrol_build_current", "\u4ECE\u5F53\u524D STPD \u6570\u636E\u6784\u5EFA",
                                  class = "btn-primary", width = "100%"),
                     fileInput("patrol_bundle_in", "\u6216\u8F7D\u5165 reviewer bundle (.rds)",
                               accept = c(".rds", "application/octet-stream"))
                   ),
                   column(
                     5,
                     textInput("patrol_annotation_dir", "\u4EBA\u5DE5\u6807\u6CE8\u4FDD\u5B58\u76EE\u5F55",
                               value = file.path(normalizePath(getwd()),
                                                 "stpd_patrol_annotations")),
                     tags$div(class = "small-note",
                              "\u5F53\u524D\u6570\u636E\u5165\u53E3\u4F1A\u8FD0\u884C STPD\u3001Mean-ISI\u3001LogISI/newBD\u3001PS \u548C RGS \u8BC1\u636E\u6574\u5408\uFF1B\u5927\u6570\u636E\u96C6\u53EF\u80FD\u9700\u8981\u7B49\u5F85\u3002")
                   ),
                   column(3, uiOutput("patrol_integrated_status"))
                 ),
                 tags$hr(),
                 uiOutput("patrol_integrated_panel")),
