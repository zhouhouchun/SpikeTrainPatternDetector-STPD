...
#
# Implements a pCLAMP-style Legendy-Salcman Poisson-surprise algorithm:
...
# matching the event-rate convention used by the implemented pCLAMP-style
# Poisson-surprise variant.
#
# This file uses only base R.

STPD_PS_IMPLEMENTATION_VARIANT <- paste0(
  "pclamp_style_m_minus_1_half_mean_seed_",
  "right_extend_front_trim_v1"
)
...
#' Detect bursts using a pCLAMP-style Poisson-surprise algorithm
#'
...
#'   This implementation defaults to 3. Some basal-ganglia protocols require 4 spikes
...
#'   train-wide mean ISI. The pCLAMP-style default used here is 0.5.
...
#'   multiple of the train-wide mean ISI. The pCLAMP-style default used here is 2.
...
#'   may fail to improve surprise before extension stops. The default used here is 10.
#' @param rate_method "mean_isi" uses 1/mean(ISI), matching common pCLAMP-style
...
stpd_detect_poisson_surprise <- function(
  ...
  parameters <- data.frame(
    implementation_variant = STPD_PS_IMPLEMENTATION_VARIANT,
    surprise_threshold = surprise_threshold,
    surprise_log_base = log_base,
...
print.poisson_surprise_result <- function(x, ...) {
  cat("pCLAMP-style Legendy-Salcman Poisson-surprise burst detection\n")
...
stpd_poisson_surprise_support_dataset <- function(
  ...
  bind_rows <- function(x) {
    frames <- x[vapply(x, is.data.frame, logical(1))]
    if (!length(frames)) return(data.frame())
    keep <- vapply(frames, function(z) nrow(z) > 0L, logical(1))
    if (!any(keep)) return(frames[[1L]][0, , drop = FALSE])
    out <- do.call(rbind, frames[keep])
    rownames(out) <- NULL
    out
  }
...
stpd_poisson_surprise_support_export <- function(support, out_dir) {
  ...
  tables <- list(
    ...
    PS_run_status.csv = data.frame(
      status = if (nrow(support$bursts) == 0L) "zero_events" else "success",
      implementation_variant = support$implementation_variant,
