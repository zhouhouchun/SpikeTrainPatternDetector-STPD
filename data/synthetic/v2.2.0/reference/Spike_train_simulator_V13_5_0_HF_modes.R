...
simulate_stimulus_sequence_core <- function(config, seed = NULL) {
  ...
  baseline_label_candidates <- function(use_recovery_mode = FALSE, excluded = character(0)) {
    ...
    if (isTRUE(config$avoid_consecutive_burst_runs) &&
        identical(last_label, "Burst")) {
      base <- setdiff(base, "Burst")
    }
  ...
  order_baseline_candidates <- function(candidates) {
    ...
    # Weighted random permutation of every feasible label.  Keeping the first
    # configured label fixed made Burst the first attempted run on every loop,
    # so short-scale simulations accumulated implausible Burst saturation.
    w <- ratios[candidates]
    w[!is.finite(w) | w < 0] <- 0
    if (length(candidates) == 1L) return(candidates)
    if (sum(w) > 0) {
      sample(candidates, length(candidates), prob = w, replace = FALSE)
    } else {
      sample(candidates, length(candidates), replace = FALSE)
    }
  }
  ...
  append_label_run <- function(label, n_intervals, source = "baseline", stimulus_id = NA_integer_,
    ...
    n_intervals <- max(1L, as.integer(n_intervals))
    if (isTRUE(config$avoid_consecutive_burst_runs) &&
        identical(label, "Burst") && identical(last_label, "Burst")) {
      last_append_failure_reason <<- "consecutive_burst_run_forbidden"
      return(FALSE)
    }
...
simulate_spike_train_core <- function(config, seed = NULL) {
  ...
  choose_interval_label <- function(excluded = character(0)) {
    ...
    # Same-label continuation remains available for States and background, but
    # benchmark Burst runs may not concatenate into an episode larger than the
    # configured Burst support merely because the scheduler sampled Burst twice.
    if (isTRUE(config$avoid_consecutive_burst_runs) &&
        identical(last_isi_pattern, "Burst")) {
      weights["Burst"] <- 0
    }
  ...
  append_interval_run <- function(pattern, n_intervals, fixed_durations = NULL, source = "manual") {
    ...
    if (isTRUE(config$avoid_consecutive_burst_runs) &&
        identical(pattern, "Burst") && identical(last_isi_pattern, "Burst")) {
      return(FALSE)
    }
