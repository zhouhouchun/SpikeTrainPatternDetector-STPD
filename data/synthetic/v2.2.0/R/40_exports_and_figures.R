scale_table_v22 <- function(data, sample_key, time_columns) {
...
make_sample_key_v22 <- function(template_manifest) {
...
canonicalize_xlsx_archive <- function(path) {
  ...
  core <- gsub("<dcterms:created[^>]*>[^<]*</dcterms:created>",
               "<dcterms:created xsi:type=\"dcterms:W3CDTF\">2026-08-28T00:00:00Z</dcterms:created>", core)
  ...
  Sys.setFileTime(file.path(stage, files), as.POSIXct("2026-08-28 00:00:00", tz = "UTC"))
...
write_detector_workbook_v22 <- function(detector_input, sample_key, path) {
...
select_calibration_v22 <- function(events_u, states_u, sample_key) {
...
make_distribution_summary_v22 <- function(intervals, events, states, separators) {
  ...
  out <- rbind(event_summary, state_summary, separator_summary); rownames(out) <- NULL; out
}

make_plots_v22 <- function(intervals, spikes, evidence, quota, output_dir) {
  x <- intervals
  ...
  x$Secondary_Rail <- ifelse(x$Contextual_Separator_Label == parameters$contextual_separator$secondary_label,
                             "Secondary: contextual separator", "Secondary: none")
  ...
  base_plot <- ggplot2::ggplot(anchor) +
    ...
    ggplot2::labs(x = "Dimensionless time (B units)", y = NULL, color = "Frozen truth",
                  title = "Twenty dimensionless v2.2 spike-train templates",
                  subtitle = "Black ticks: spikes | upper: State | middle: primary Event | lower: secondary contextual separator") +
  ...
  p3 <- base_plot +
    ...
    ggplot2::labs(title = "Mechanism truth and independently audited observable phenotype",
                  subtitle = "State, primary Event and contextual separator remain distinct; thin bottom marks phenotype evidence")
