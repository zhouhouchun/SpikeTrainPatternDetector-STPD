...
public enum STPDLocalization {
    ...
    public static let exactDictionary: [String: String] = [
        ...
        "正在运行基线和逐家族留出检测…": "Running the baseline and separate per-family held-out detections…",
        ...
        "当前参数作为基线；每个学习家族在留出 train 上单独运行，人工留出标签不会反馈到检测器。": "The current parameters are the baseline; each learned family is run separately on held-out trains, and held-out manual labels are never fed back into the detector.",
        ...
        "已审核 ISI %d · train %d": "Reviewed ISIs %d · trains %d",
        "几何完整 %d/%d": "complete geometry %d/%d",
        "FP Δ %+d · FN Δ %+d · 拆分 Δ %+d · 合并 Δ %+d":
            "FP Δ %+d · FN Δ %+d · splits Δ %+d · merges Δ %+d",
        "仅报告": "Report only",
        "可经确认应用": "Eligible for explicit application",
        "没有可写入的兼容阈值": "No compatible writable threshold",
        "校准证据少于两条 train": "Calibration evidence covers fewer than two trains",
        "校准内部跨 train 验证不可用": "Internal cross-train calibration validation is unavailable",
        "校准内部跨 train 验证未一致通过": "Internal cross-train calibration validation did not pass consistently",
        "留出集没有已审核 ISI": "No reviewed ISIs in the held-out set",
        "留出集没有该家族阳性支持": "No positive support for this family in the held-out set",
        "留出集没有显式阴性对照": "No explicit negative controls in the held-out set",
        "留出 train 未完成整条几何复核": "At least one held-out train lacks a complete whole-train geometry review",
        "至少一条留出 train 出现指标退化": "At least one held-out train has a measured regression",
        "F1 下降": "F1 decreased",
        "分段 IoU 下降": "Segment IoU decreased",
        "边界误差增加": "Boundary error increased",
        "假阳性增加": "False positives increased",
        "假阴性增加": "False negatives increased",
        "错误拆分增加": "False splits increased",
        "错误合并增加": "False merges increased",
        "未观察到指标改善": "No metric improvement was observed",
        "至少一项指标改善，且无已测指标退化":
            "At least one metric improved with no measured regression",
        "；": "; ",
        "导出验证报告": "Export validation report",
        "应用所选准入家族": "Apply selected admitted families",
        "只写入该次校准 train 产生、且留出指标无已测退化并至少有一项改善的家族；仍需用户确认并重新运行检测。":
            "Writes only families learned from this calibration split with no measured held-out regression and at least one improvement; user confirmation and a detector re-run are still required.",
        "每次只应用一个单独验证的家族；应用后必须重新运行留出验证，才能继续应用其他家族。":
            "Apply only one independently validated family at a time; after application, re-run held-out validation before applying another family.",
        "所选准入家族仍包含需要确认的科学警告":
            "The selected admitted families still contain scientific warnings that require confirmation",
        "确认应用所选准入家族": "Confirm applying selected admitted families",
        "准入只表示本次 train 留出比较中没有已测退化且观察到改善，不等于外部生物学验证。请确认后再写入参数。":
            "Admission means only that this train-held-out comparison observed improvement without measured regression; it is not external biological validation. Confirm before writing parameters.",
        "当前没有可导出的有效留出验证报告。": "There is no valid held-out validation report to export.",
        "当前没有有效的留出验证报告。": "There is no valid held-out validation report.",
        "导出身份绑定的逐模式留出验证报告。CSV 与 XLSX 内容完全一致；报告不会改变检测器参数。":
            "Export an identity-bound per-pattern held-out validation report. CSV and XLSX contain identical data; the report does not change detector parameters.",
        "导出格式": "Export format",
        "CSV — 单表验证报告": "CSV — single-table validation report",
        "XLSX — 单工作表验证报告": "XLSX — single-sheet validation report",
        "留出验证报告导出已取消。": "Held-out validation report export was cancelled.",
        "保存期间报告或检测参数发生变化；请重新运行验证。":
            "The report or detector parameters changed while saving; re-run validation.",
        "已导出留出验证报告：%@": "Exported held-out validation report: %@",
        "留出验证报告导出失败。": "Held-out validation report export failed.",
        "没有选择可明确应用的留出验证家族。": "Select exactly one held-out-admitted family to apply.",
        "当前参数无法组成已验证的单家族应用配置；请重新运行留出验证。": "The current parameters cannot form the validated one-family application profile; re-run held-out validation.",
        "应用后的参数与已验证配置不一致；已安全回滚。": "The applied parameters did not match the validated profile and were safely rolled back.",
        "已应用一个经留出验证的家族；如需应用另一家族，请先重新运行验证。": "Applied one held-out-validated family; re-run validation before applying another family.",
        "Could not create the XLSX report.": "Could not create the XLSX report.",
    ]
    ...
    public static let russianExactDictionary: [String: String] = [
        ...
        "正在运行基线和逐家族留出检测…": "Выполняются базовая и отдельные по семействам детекции на отложенных данных…",
        ...
        "当前参数作为基线；每个学习家族在留出 train 上单独运行，人工留出标签不会反馈到检测器。": "Текущие параметры служат базовой линией; каждое обученное семейство отдельно проверяется на отложенных train, а ручные отложенные метки не передаются детектору.",
        ...
        "已审核 ISI %d · train %d": "Проверено ISI: %d · train: %d",
        "几何完整 %d/%d": "полная геометрия %d/%d",
        "FP Δ %+d · FN Δ %+d · 拆分 Δ %+d · 合并 Δ %+d":
            "FP Δ %+d · FN Δ %+d · разделения Δ %+d · слияния Δ %+d",
        "仅报告": "Только отчёт",
        "可经确认应用": "Можно применить после подтверждения",
        "没有可写入的兼容阈值": "Нет совместимого порога для записи",
        "校准证据少于两条 train": "Калибровочные данные охватывают менее двух train",
        "校准内部跨 train 验证不可用": "Внутренняя меж-train проверка калибровки недоступна",
        "校准内部跨 train 验证未一致通过": "Внутренняя меж-train проверка калибровки не дала согласованного результата",
        "留出集没有已审核 ISI": "В отложенном наборе нет проверенных ISI",
        "留出集没有该家族阳性支持": "В отложенном наборе нет положительной поддержки этого семейства",
        "留出集没有显式阴性对照": "В отложенном наборе нет явного отрицательного контроля",
        "留出 train 未完成整条几何复核": "Хотя бы для одной отложенной train не завершена геометрическая проверка всей последовательности",
        "至少一条留出 train 出现指标退化": "Хотя бы на одной отложенной train наблюдается ухудшение метрик",
        "F1 下降": "F1 снизился",
        "分段 IoU 下降": "IoU сегментов снизился",
        "边界误差增加": "Ошибка границ увеличилась",
        "假阳性增加": "Число ложноположительных результатов увеличилось",
        "假阴性增加": "Число ложноотрицательных результатов увеличилось",
        "错误拆分增加": "Число ложных разделений увеличилось",
        "错误合并增加": "Число ложных слияний увеличилось",
        "未观察到指标改善": "Улучшения метрик не наблюдалось",
        "至少一项指标改善，且无已测指标退化":
            "Как минимум одна метрика улучшилась без измеренного ухудшения",
        "；": "; ",
        "导出验证报告": "Экспортировать отчёт проверки",
        "应用所选准入家族": "Применить выбранные допущенные семейства",
        "只写入该次校准 train 产生、且留出指标无已测退化并至少有一项改善的家族；仍需用户确认并重新运行检测。":
            "Записываются только семейства из этой калибровки без измеренного ухудшения на отложенных данных и как минимум с одним улучшением; всё равно нужны подтверждение пользователя и повторный запуск детектора.",
        "每次只应用一个单独验证的家族；应用后必须重新运行留出验证，才能继续应用其他家族。":
            "За один раз применяйте только одно независимо проверенное семейство; перед применением другого повторите отложенную проверку.",
        "所选准入家族仍包含需要确认的科学警告":
            "Для выбранных допущенных семейств остаются научные предупреждения, требующие подтверждения",
        "确认应用所选准入家族": "Подтвердить применение выбранных допущенных семейств",
        "准入只表示本次 train 留出比较中没有已测退化且观察到改善，不等于外部生物学验证。请确认后再写入参数。":
            "Допуск означает лишь улучшение без измеренного ухудшения в этом сравнении с отложенными train; это не внешняя биологическая валидация. Подтвердите запись параметров.",
        "当前没有可导出的有效留出验证报告。": "Нет действительного отчёта отложенной проверки для экспорта.",
        "当前没有有效的留出验证报告。": "Нет действительного отчёта отложенной проверки.",
        "导出身份绑定的逐模式留出验证报告。CSV 与 XLSX 内容完全一致；报告不会改变检测器参数。":
            "Экспорт отчёта отложенной проверки по каждому паттерну, привязанного к идентичности. CSV и XLSX содержат одинаковые данные; отчёт не меняет параметры детектора.",
        "导出格式": "Формат экспорта",
        "CSV — 单表验证报告": "CSV — единая таблица проверки",
        "XLSX — 单工作表验证报告": "XLSX — один лист проверки",
        "留出验证报告导出已取消。": "Экспорт отчёта отложенной проверки отменён.",
        "保存期间报告或检测参数发生变化；请重新运行验证。":
            "Во время сохранения изменились отчёт или параметры детектора; повторите проверку.",
        "已导出留出验证报告：%@": "Отчёт отложенной проверки экспортирован: %@",
        "留出验证报告导出失败。": "Не удалось экспортировать отчёт отложенной проверки.",
        "没有选择可明确应用的留出验证家族。": "Выберите ровно одно допущенное по отложенной проверке семейство.",
        "当前参数无法组成已验证的单家族应用配置；请重新运行留出验证。": "Текущие параметры не образуют проверенный профиль одного семейства; повторите отложенную проверку.",
        "应用后的参数与已验证配置不一致；已安全回滚。": "Применённые параметры не совпали с проверенным профилем и были безопасно отменены.",
        "已应用一个经留出验证的家族；如需应用另一家族，请先重新运行验证。": "Применено одно семейство, проверенное на отложенных данных; перед другим семейством повторите проверку.",
        "Could not create the XLSX report.": "Не удалось создать отчёт XLSX."
    ]
