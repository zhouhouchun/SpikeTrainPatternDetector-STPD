...
test_that("Poisson-surprise support exports the standard result tables", {
  ...
  expect_setequal(
    ...
    c(
      ...
      "PS_thresholds.csv", "PS_run_status.csv"
    )
  )
...
test_that("Poisson-surprise zero-event export retains the complete schema", {
  dataset <- SpikeTrainPatternDetector:::make_dataset(
    "ps-zero-export", "synthetic",
    list(t1 = data.frame(timestamp_sec = seq(0, 2, by = 0.01)))
  )
  support <- stpd_poisson_surprise_support_dataset(
    dataset, surprise_threshold = 3, log_base = "10"
  )
  expect_equal(nrow(support$bursts), 0L)
  expect_true(all(c("train", "start_isi", "end_isi", "n_spikes") %in%
                    names(support$bursts)))
  out <- tempfile("ps_zero_export_")
  stpd_poisson_surprise_support_export(support, out)
  expect_true(file.exists(file.path(out, "PS_burst_features.csv")))
  status <- utils::read.csv(file.path(out, "PS_run_status.csv"))
  expect_identical(status$status, "zero_events")
})
